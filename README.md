...
`-CoreGroup` 只允许与 `-Suite core` 一起使用，默认值 `all` 保持 45 个 core 脚本及其顺序；五个可单独执行的分组依次为 `entry-lifecycle`（14）、`evaluation-release`（10）、`install-evidence`（3）、`governance-approval`（3）和 `harness-contracts`（15）。从 Windows PowerShell 5.1 进入时，该参数也会透明转交给 PowerShell 7 runner。
