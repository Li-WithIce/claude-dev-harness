...
Only a new task still selected as `auto` reads Canonical `.assistant/runtime/protocol-default.json`. The strict `harness-runtime-default/v1` document contains only the selected protocol, release or workspace-canary scope, source revision, decision identity/time, optional workspace/expiry binding, required capabilities, and digest. Runtime validates those fields against its current distribution and actual capability facts; it does not load a Qualification Report or reconstruct how the decision was earned. Missing, invalid, expired, source-drifted, workspace-mismatched, or capability-incompatible decisions fall back to v1. Workspace `enable-v2` and `HARNESS_PROTOCOL=v2` remain independent opt-ins and do not assert that any release gate passed.
