...
Generator consumes one strict `rollout-evidence-set/v1` containing the phase, current source revision, exact Host binding and the 18 non-G12 Gate references. Each reference requires the evidence Contract, Artifact path, raw file digest, source revision, and producer/receipt identity. Caller-declared status is never authoritative: Generator reopens the ordinary file, verifies raw bytes/digest, and delegates status derivation to the named strict Adapter. Strict Portable Adapters are wired for G01, G02, G03, G05, G06, G07, G14, G18, G19 and G20. G00, G04, G09, G10, G11, the Release Workflow and credential-blind aggregation remain later DP-02C work; G13, G15 and G16 remain later Promotion/Canary/Stable work. Real Model40, Host, and formal G14 evidence has not been run, so no formal Gate pass is claimed. Hand-written/fixture JSON cannot produce a promotable report, and the former Model/Host parameters remain historical-only failures.
