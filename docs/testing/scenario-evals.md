...
- The existing workflow still describes the pre-DP-02A sequential model/host jobs and legacy two-input `release-full` command. The v2 Generator rejects that old command, reopens every `rollout-evidence-set/v1` Artifact, and fails with provenance unavailable while formal DP-02B/DP-02C contract Adapters remain unwired. A non-authorizing Review Payload and a separate phase-bound Review Receipt cannot upgrade fixture or hand-written status into release qualification. DP-02A does not route the Default Promotion branch, run release jobs, or turn deterministic scenario evals, fixture benchmarks, local validation, or lifecycle smokes into formal Evidence. Later producer adapters must preserve the 120/240/120-minute budgets, dedicated producer/aggregator account boundary, exact Codex `0.144.4` Observed Host Context binding, fixed sanitized artifacts, and `!cancelled()` semantics before they may construct the strict normalized Gate set.
