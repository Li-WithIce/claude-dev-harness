...
function Invoke-HarnessRuntimeGit {
    ...
    $output = @(& git -C $RepoRoot @GitArguments 2>$null | ForEach-Object { [string]$_ })
...
function Get-HarnessRuntimeSourceIdentity {
    ...
    try {
        ...
        $source = @(Invoke-HarnessRuntimeGit -RepoRoot $root -GitArguments @('show','-s','--format=format:%H%n%T','HEAD') -FailureReason 'runtime-default-source-revision-unavailable')
        if ($source.Count -ne 2 -or $source[0] -cnotmatch '^[0-9a-f]{40,64}$' -or $source[1] -cnotmatch '^[0-9a-f]{40,64}$') {
            throw 'runtime-default-source-revision-unavailable'
        }

        $indexLines = @(Invoke-HarnessRuntimeGit -RepoRoot $root -GitArguments (@('ls-files','--stage','-v','--') + $script:RuntimeSourcePaths) -FailureReason 'runtime-default-source-index-unavailable')
        $statusLines = @(Invoke-HarnessRuntimeGit -RepoRoot $root -GitArguments (@('status','--porcelain=v2','--untracked-files=all','--') + $script:RuntimeSourcePaths) -FailureReason 'runtime-default-source-status-unavailable')
    } finally {
        [Environment]::SetEnvironmentVariable('GIT_OPTIONAL_LOCKS',$previousGitOptionalLocks,[EnvironmentVariableTarget]::Process)
    }

    if (@($statusLines | Where-Object { $_.StartsWith('? ',[StringComparison]::Ordinal) }).Count -gt 0) {
        throw 'runtime-default-source-untracked-shadow'
    }
    ...
    foreach ($line in $indexLines) {
        ...
        if (-not $match.Success -or $match.Groups['flag'].Value -cne 'H' -or $match.Groups['mode'].Value -cne '100644' -or $match.Groups['stage'].Value -cne '0') {
            throw 'runtime-default-source-unsafe-index'
        }
        if (-not $indexedPaths.Add($match.Groups['path'].Value)) { throw 'runtime-default-source-unsafe-index' }
    }
    if ($indexedPaths.Count -ne $script:RuntimeSourcePaths.Count -or @($script:RuntimeSourcePaths | Where-Object { -not $indexedPaths.Contains($_) }).Count -gt 0) {
        throw 'runtime-default-source-path-missing'
    }

    $entries = @($script:RuntimeSourcePaths | ForEach-Object {
        $relative = [string]$_
...
function Assert-HarnessRuntimeDefaultDecision {
    ...
    $required = @('schema_version','new_task_protocol','scope','source_revision','source_identity','decision_id','issued_at_utc','required_capabilities','decision_digest')
