...
$script:RuntimeSourcePaths = @(
    ...
    'scripts/lib/Harness.RuntimeDefaultReader.ps1'
...
function Get-HarnessRuntimeSourceIdentity {
    ...
    foreach ($line in (Invoke-HarnessGit -WorkspaceRoot $root -Arguments (@('ls-files','--stage','-v','--') + $script:RuntimeSourcePaths) -FailureReason 'runtime-default-source-index-unavailable').Lines) {
    ...
    return [ordered]@{schema_version = 'harness-runtime-source-identity/v1'
        ...
        runtime_contract_digest = Get-HarnessUtf8TextSha256 -Text (([ordered]@{
            schema_version='harness-runtime-contract/v1'
            paths=@($script:RuntimeSourcePaths | ForEach-Object {
...
function Assert-HarnessRuntimeDefaultDecision {
    ...
    foreach ($binding in ([ordered]@{revision='revision';tree_oid='tree'
        path_set_digest='path-set';runtime_contract_digest='contract'}).GetEnumerator()) {
        if ([string]$Document.source_identity[$binding.Key] -cne [string]$ObservedSourceIdentity[$binding.Key]) { throw "runtime-default-source-$($binding.Value)-mismatch" }
    }
    ...
    if ([string]$Document.scope -ceq 'workspace-canary') {
        ...
        if (-not [datetimeoffset]::TryParse([string]$Document.expires_at_utc,[Globalization.CultureInfo]::InvariantCulture,[Globalization.DateTimeStyles]::RoundtripKind,[ref]$expiresAt) -or $expiresAt -le $issuedAt) { throw 'runtime-default-expiry-invalid' }
...
function Get-HarnessRuntimeDefaultDecision {
    ...
    $result = [ordered]@{status='missing';usable=$false;reason='runtime-default-missing'
        ...
        scope=$null;required_capabilities=@();missing_capabilities=@()
    ...
    try {
        ...
        $result.status,$result.reason = 'invalid','runtime-default-invalid'
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'runtime-default-not-regular-file' }
        try {
            $document = Read-HarnessKernelJson -Path $path -Label 'runtime default' -MaximumBytes 64KB
        } catch {
        ...
        foreach ($key in @('decision_digest','new_task_protocol','scope')) { $result[$key] = [string]$document[$key] }
        $result.required_capabilities,$result.host_capabilities = @($document.required_capabilities),$HostCapabilities
        if ($missing.Count -gt 0) {
            $result.status,$result.reason,$result.missing_capabilities = 'unavailable',('runtime-default-capability-missing-' + ($missing -join ',')),$missing
        } else {
            $result.status,$result.usable,$result.reason = 'valid',$true,('runtime-default-' + [string]$document.new_task_protocol)
        }
    } catch {
        $result.status,$result.reason = 'invalid',$(if ([string]$_.Exception.Message.StartsWith('runtime-default-',[StringComparison]::Ordinal)) { [string]$_.Exception.Message } else { 'runtime-default-invalid' })
    }
    return $result
}
