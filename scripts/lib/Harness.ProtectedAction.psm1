...
function Read-HarnessProtectedPolicy {
    ...
    if (Test-Path -LiteralPath $overlayPath) {
        ...
        try { $overlay = Read-HarnessKernelJson -Path $overlayPath -Label 'protected action overlay' } catch { throw "protected action overlay is invalid: $($_.Exception.Message)" }
        try {
...
function Read-HarnessProtectedTask {
    ...
    try{$contract=Resolve-HarnessRequirementContract -RepoRoot $RepoRoot -WorkspaceRoot $WorkspaceRoot -TaskId $TaskId -Path $task.contract_path -Label 'protected task Contract'}catch{
        throw $(if($_.Exception.Message-match'digest does not match canonical content'){'protected task Contract digest is stale'}else{"protected task Contract is invalid: $($_.Exception.Message)"})
    }
...
function Assert-HarnessProtectedAction {
    ...
    $unprotected=[ordered]@{allowed=$true;protected=$false;
        matched_rules=@();required_scopes=@();
        approval_id=$null;operation_identity=$null;
        protected_operation=$null}
    if($ActionMode-ceq'read'){return $unprotected}
    $normalizedPaths=@($ChangedPaths|ForEach-Object{
        Get-HarnessRelativePath -WorkspaceRoot $WorkspaceRoot -Path $_
    })
    $matched,$scopes=[Collections.Generic.List[object]]::new(),[Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach($rule in @((Read-HarnessProtectedPolicy -RepoRoot $RepoRoot -WorkspaceRoot $WorkspaceRoot).rules)){
    ...
    if($matched.Count-eq0){return $unprotected}
    ...
    $requiredType=$(if($requiredTypes.Count-eq1){[string]$requiredTypes[0]}else{'none'})
    ...
    if(@{governed=1;critical=2}[[string]$task.execution_profile]-lt@{governed=1;critical=2}[$(if(@($matched.requires_profile)-ccontains'critical'){'critical'}else{'governed'})]){throw 'protected write task profile is insufficient'}
    $operation=New-HarnessProtectedOperation -TaskVersion ([int]$task.version) -ContractDigest ([string]$task.contract_digest) `
        -Environment $(if([string]::IsNullOrWhiteSpace($Environment)){'workspace'}else{$Environment}) -ActionCategory protected-write `
        -Targets $(if($normalizedPaths.Count){@($normalizedPaths)}else{@('workspace')}) -ApprovalType $requiredType -ApprovalScope @($scopes)
    $approval=if($requiredType-ceq'none'){$null}else{
    ...
    return [ordered]@{allowed=$true;protected=$true;
        matched_rules=@($matched|ForEach-Object{[string]$_.id});required_scopes=@($scopes|Sort-Object);
        approval_id=$(if($null-ne$approval){[string]$approval.Document.approval_id}else{$null});operation_identity=[string]$operation.identity;
