...
[CmdletBinding()]
param(
    [Parameter(Mandatory = $true)][string]$WorkspaceRoot,[string]$RepoRoot = "",
    [string]$UserProfileRoot = "",[switch]$ProbeHostDetails
)
...
try {
    ...
    $summary = [pscustomobject]@{status=$status;RepoRoot=$RepoRoot;WorkspaceRoot=$WorkspaceRoot;host_details_probed=[bool]$ProbeHostDetails;workspace_protocol=$(if ($null -eq $resolution.workspace_config.new_task_protocol) { 'not-read' } else { $resolution.workspace_config.new_task_protocol })}
    $projection = [pscustomobject]@{summary=$summary;host=$hostCapabilities;resolution=$resolution;policy=$protectedPolicy}
    ...
    exit @('PASS','WARN','FAIL').IndexOf($status)
} catch {
