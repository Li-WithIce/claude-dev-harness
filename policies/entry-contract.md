...
- Resolve project/task identity before mode; identity never broadens authorization.
- For project-scoped development and read-only engineering requests, `entry-router` is the default first hop. Do not invoke other workflow skills before routing; after routing, load only the context required by the selected mode.
