...
Check ([int]$inventory.budget.baseline_executable_loc -eq 2995 -and [int]$inventory.budget.current_executable_loc -eq 2995 -and [int]$inventory.budget.current_executable_loc -lt 3000 -and [int]$inventory.budget.delta -eq 0 -and [int]$inventory.budget.covered_growth -eq 0 -and [string]$inventory.budget.status -ceq 'within-baseline' -and $budgetExceptions.Count -eq 0) 'TK-07 ratchets Runtime to 2995, below the 3000-line target, with no exception' 'Runtime budget, Adapter Action closure, or exception state drifted'
...
Check ([int]$inventory.totals.file_count -eq @($inventory.files).Count -and [int]$inventory.totals.artifact_count -eq @($inventory.artifacts).Count -and [int]$inventory.totals.runtime_executable_loc -eq 2995) 'inventory totals bind the current TK-07 Runtime measurement' 'inventory totals or Runtime measurement drifted'
