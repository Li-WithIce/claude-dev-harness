...
Check ([int]$inventory.budget.baseline_executable_loc -eq 6151 -and [int]$inventory.budget.current_executable_loc -eq 6150 -and [int]$inventory.budget.delta -eq -1 -and [int]$inventory.budget.covered_growth -eq 0 -and [string]$inventory.budget.status -ceq 'within-baseline' -and $budgetExceptions.Count -eq 0) 'TK-05 keeps Runtime below the 6151 ceiling with a measured 6150 and no exception' 'Runtime budget, Adapter Action closure, or exception state drifted'
...
Check ([int]$inventory.totals.file_count -eq @($inventory.files).Count -and [int]$inventory.totals.artifact_count -eq @($inventory.artifacts).Count -and [int]$inventory.totals.runtime_executable_loc -eq 6150) 'inventory totals bind the current TK-05 Runtime measurement' 'inventory totals or Runtime measurement drifted'
