...
Check ([int]$inventory.budget.baseline_executable_loc -eq 6151 -and [int]$inventory.budget.current_executable_loc -eq 6178 -and [int]$inventory.budget.delta -eq 27 -and [int]$inventory.budget.covered_growth -eq 27 -and [string]$inventory.budget.status -ceq 'covered-by-exception' -and $budgetExceptions.Count -eq 1 -and [string]$budgetExceptions[0].id -ceq 'KTB-EX-001' -and [int]$budgetExceptions[0].added_lines -eq 27 -and [string]$budgetExceptions[0].expires_at_milestone -ceq 'TK-01B-Compat') 'Runtime budget keeps the TK-00 baseline and exactly covers TK-01A measured growth' 'Runtime budget baseline, measured growth, or TK-01A exception drifted'
...
Check ([int]$inventory.totals.file_count -eq @($inventory.files).Count -and [int]$inventory.totals.artifact_count -eq @($inventory.artifacts).Count -and [int]$inventory.totals.runtime_executable_loc -eq 6178) 'inventory totals bind the current TK-01A Runtime measurement' 'inventory totals or Runtime measurement drifted'
