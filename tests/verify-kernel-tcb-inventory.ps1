...
Check ([int]$inventory.totals.file_count -eq @($inventory.files).Count -and [int]$inventory.totals.artifact_count -eq @($inventory.artifacts).Count -and [int]$inventory.totals.runtime_executable_loc -eq 6151) 'inventory totals bind the current TK-01B-Compat Runtime measurement' 'inventory totals or Runtime measurement drifted'
