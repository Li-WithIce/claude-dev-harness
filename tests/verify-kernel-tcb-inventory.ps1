...
Check ($runtimePaths -contains 'scripts/lib/Harness.Path.psm1' -and $runtimePaths -contains 'scripts/lib/Harness.AtomicWrite.psm1') 'Runtime TCB contains canonical Path and AtomicWrite primitives' 'canonical K0 primitives are missing from Runtime TCB'
...
Check ([int]$inventory.budget.baseline_executable_loc -eq [int]$inventory.budget.current_executable_loc -and [int]$inventory.budget.delta -eq 0 -and [string]$inventory.budget.status -ceq 'within-baseline' -and @($inventory.budget.exceptions).Count -eq 0) 'Runtime budget baseline is a zero-delta ratchet with no exception' 'Runtime budget has drift or an unexplained exception'
...
Check ([int]$inventory.totals.file_count -eq @($inventory.files).Count -and [int]$inventory.totals.artifact_count -eq @($inventory.artifacts).Count -and [int]$inventory.totals.runtime_executable_loc -eq 6151) 'inventory totals bind the current TK-00 Runtime baseline' 'inventory totals or Runtime baseline drifted'
