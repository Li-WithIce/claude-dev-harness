...
try {
    ...
    Check ($modelJob -match [regex]::Escape($defaultPromotionCondition) -and $modelJob -match 'write-release-producer-receipt\.ps1[^\r\n]+-Kind model\b' -and $modelJob -match 'release-model-receipt\.json' -and $hostJob -match [regex]::Escape($defaultPromotionCondition) -and $hostJob -match 'write-release-producer-receipt\.ps1[^\r\n]+-Kind host\b' -and $hostJob -match 'release-host-receipt\.json' -and $fullJob -notmatch 'refs/heads/codex/harness-v2-default-promotion|release-(?:model|host)-receipt\.json') 'Release Workflow binds manual-only G09/G10 Producers while G11 remains unwired' 'Release Workflow Producer receipt binding or G11 boundary is wrong'
    $runtimeReaders=@('scripts/task.ps1','scripts/lib/Harness.Recovery.psm1','scripts/lib/Harness.RuntimeDefault.psm1')
    Check (@($runtimeReaders|Where-Object{(Get-Content -LiteralPath (Join-Path $RepoRoot $_) -Raw -Encoding utf8)-match'release-(?:model|host)-receipt'}).Count -eq 0) 'Runtime Core, ordinary Status, and Runtime Default do not read Release Producer Receipts' 'a Runtime path began reading Release Producer Receipts'
    $unwiredPath=Join-Path $temp 'g11-unwired.json';[IO.File]::WriteAllText($unwiredPath,'{}',[Text.UTF8Encoding]::new($false));$g11=[ordered]@{'DP-G11-RELEASE-FULL'=[ordered]@{status='pass';evidence_contract='fixture/v1';artifact_path=$unwiredPath;evidence_digest=Get-FileDigest $unwiredPath;source_revision=[string]$cleanSource.revision;producer_identity='forged-caller'}}
    ...
    Check ($g11Reason -ceq 'rollout-evidence-provenance-unwired-DP-G11-RELEASE-FULL') 'G11 remains provenance-unwired and fail closed' 'G11 was wired or stopped failing closed'
