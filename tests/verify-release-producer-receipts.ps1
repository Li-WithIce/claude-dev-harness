...
try {
    ...
    $workflowDiff=@(& git -C $RepoRoot diff --exit-code HEAD -- .github/workflows 2>&1);Check ($LASTEXITCODE -eq 0) 'Release Workflow remains byte-identical to HEAD' "Release Workflow changed: $($workflowDiff -join ' | ')"
