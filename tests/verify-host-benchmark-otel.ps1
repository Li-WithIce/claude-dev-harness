...
try {
    ...
    Check ($receiverText -match "\^/v1/traces/round-\[1-9\]\[0-9\]\*\$" -and $receiverText -match "content-type.*application/json" -and $receiverText -match 'ReadTimeout' -and $receiverText -match 'WriteTimeout' -and $receiverText -match 'MaxRequests' -and $receiverText -match 'MaxTotalBodyBytes' -and $receiverText -match 'ExpectedClientIdentityPath' -and $receiverText -match 'GetExtendedTcpTable' -and $receiverText -match 'start_time_filetime_utc' -and $receiverText -match 'FileMode\]::CreateNew' -and $receiverText -match 'FileShare\]::Read') 'bounded provenance-bound receiver contract is incomplete'
