...
try {
    ...
    Assert-True -Condition ($canonical -match '`auto_resolves_to`:\s*`existing-artifact-or-gated-v2-new`' -and $canonical -match '`v2_entry_activation`:\s*`explicit-new-or-existing-v2-or-eligible-auto-new`' -and $canonical -match 'all-pass rollout selects v2, else v1') -Success 'entry contract gates new auto v2 tasks while preserving artifact detection' -Failure 'entry contract protocol rollout widened or became ambiguous'
