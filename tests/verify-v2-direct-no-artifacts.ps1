...
try {
    ...
    Assert-True -Condition ($canonical -match '`auto_resolves_to`:\s*`v1`' -and $canonical -match '`v2_entry_activation`:\s*`explicit-v2-only`' -and $canonical -match 'Only explicit `HARNESS_PROTOCOL=v2` plus a new task') -Success 'entry contract keeps auto=v1 and gates v2 to explicit new tasks' -Failure 'entry contract protocol rollout widened or became ambiguous'
