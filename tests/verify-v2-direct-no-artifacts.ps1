...
try {
    ...
    Assert-True -Condition ($canonical -match '`auto_resolves_to`:\s*`existing-artifact-or-v1-new`' -and $canonical -match '`v2_entry_activation`:\s*`explicit-new-or-existing-v2`' -and $canonical -match '`auto` keeps new tasks on v1 through PR-13') -Success 'entry contract keeps new auto tasks on v1 while detecting existing artifacts' -Failure 'entry contract protocol rollout widened or became ambiguous'
