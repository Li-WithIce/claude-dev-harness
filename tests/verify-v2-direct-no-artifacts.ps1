...
try {
    ...
    Assert-True -Condition ($canonical -match 'Selected v2 Direct loads no `entry-router`, `orchestrator`, lifecycle skill, Memory, Team, or Provider' -and $canonical -match 'Direct writes no task/runtime/current state' -and $canonical -match 'Only a detector-selected v1 request loads `entry-router`' -and $entryRouterSkill -match 'V1 compatibility entry router' -and $entryRouterSkill -match 'Never load for selected v2 Direct') -Success 'Direct entry avoids v1 lifecycle skills and durable harness writes' -Failure 'Direct entry gained a v1 lifecycle or persistence dependency'
    Assert-True -Condition ($canonical -match 'actual commands/results, self-review, and remaining gaps' -and $canonical -match '`not_run` or unavailable never means pass') -Success 'Direct response requires actual verification, self-review, and gaps' -Failure 'Direct evidence summary permits missing or false evidence'
