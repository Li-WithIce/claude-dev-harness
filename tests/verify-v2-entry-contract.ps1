...
try {
    ...
    Assert-True -Condition ($canonicalText -match '`protocol_default`:\s*`auto`' -and $canonicalText -match '`auto_resolves_to`:\s*`existing-artifact-or-v1-new`' -and $canonicalText -match '`v2_entry_activation`:\s*`explicit-new-or-existing-v2`') -Success 'auto is artifact-first while new tasks remain v1' -Failure 'entry contract protocol detector rollout is invalid'
    Assert-True -Condition ($canonicalText -notmatch '\{(?:REPO_ROOT|VAULT_PATH|CODEX_HOME)\}' -and $canonicalText -notmatch '`auto_resolves_to`:\s*`v2`') -Success 'canonical body is host-neutral and does not activate auto=v2 for new tasks' -Failure 'canonical body contains a host token or premature v2 default activation'

    $fixture = Get-Content -LiteralPath $fixturePath -Raw -Encoding utf8 | ConvertFrom-Json -ErrorAction Stop
    Assert-True -Condition ([string]$fixture.schema_version -ceq 'thin-harness-entry-routing/v1' -and [string]$fixture.protocol.default -ceq 'auto' -and [string]$fixture.protocol.auto_resolves_to -ceq 'existing-artifact-or-v1-new' -and [string]$fixture.protocol.v2_entry_activation -ceq 'explicit-new-or-existing-v2') -Success 'route fixture preserves artifact-first auto with new-task v1' -Failure 'route fixture protocol metadata is invalid'
