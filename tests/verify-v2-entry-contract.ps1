...
try {
    ...
    Assert-True -Condition ($canonicalText -match '`protocol_default`:\s*`auto`' -and $canonicalText -match '`auto_resolves_to`:\s*`v1`' -and $canonicalText -match '`v2_entry_activation`:\s*`disabled`') -Success 'auto remains on v1 and v2 entry activation is disabled' -Failure 'PR-02 prematurely activates v2 entry routing'
    Assert-True -Condition ($canonicalText -notmatch '\{(?:REPO_ROOT|VAULT_PATH|CODEX_HOME)\}' -and $canonicalText -notmatch '(?i)HARNESS_PROTOCOL\s*=\s*v2|(?:task|current)\.json') -Success 'canonical body is host-neutral and does not enable v2 runtime state' -Failure 'canonical body contains a host token or v2 runtime activation'

    $fixture = Get-Content -LiteralPath $fixturePath -Raw -Encoding utf8 | ConvertFrom-Json -ErrorAction Stop
    Assert-True -Condition ([string]$fixture.schema_version -ceq 'thin-harness-entry-routing/v1' -and [string]$fixture.protocol.default -ceq 'auto' -and [string]$fixture.protocol.auto_resolves_to -ceq 'v1' -and [string]$fixture.protocol.v2_entry_activation -ceq 'disabled') -Success 'v1 route fixture declares the inert compatibility protocol' -Failure 'v1 route fixture protocol metadata is invalid'
