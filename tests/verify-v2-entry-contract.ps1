...
try {
    ...
    Assert-True -Condition ($canonicalText -match '`protocol_default`:\s*`auto`' -and $canonicalText -match '`auto_resolves_to`:\s*`v1`' -and $canonicalText -match '`v2_entry_activation`:\s*`explicit-v2-only`') -Success 'auto remains on v1 and only explicit v2 activation is enabled' -Failure 'entry contract widened v2 activation beyond explicit opt-in'
    Assert-True -Condition ($canonicalText -notmatch '\{(?:REPO_ROOT|VAULT_PATH|CODEX_HOME)\}' -and $canonicalText -notmatch '(?:task|current)\.json' -and $canonicalText -notmatch '`auto_resolves_to`:\s*`v2`') -Success 'canonical body is host-neutral and does not activate v2 runtime or auto=v2' -Failure 'canonical body contains a host token or premature v2 runtime/default activation'

    $fixture = Get-Content -LiteralPath $fixturePath -Raw -Encoding utf8 | ConvertFrom-Json -ErrorAction Stop
    Assert-True -Condition ([string]$fixture.schema_version -ceq 'thin-harness-entry-routing/v1' -and [string]$fixture.protocol.default -ceq 'auto' -and [string]$fixture.protocol.auto_resolves_to -ceq 'v1' -and [string]$fixture.protocol.v2_entry_activation -ceq 'explicit-v2-only') -Success 'v1 route fixture preserves auto=v1 with explicit-only v2 opt-in' -Failure 'v1 route fixture protocol metadata is invalid'
