...
try {
    ...
    Check ($totals.manifest_count -eq 13 -and $totals.module_count -eq 13 -and $totals.v0_module_count -eq 6 -and $totals.v1_module_count -eq 7 -and $totals.capability_source_count -eq 7 -and $totals.capability_source_file_reference_count -eq 178 -and $totals.owner_test_count -eq 89 -and $totals.core_test_count -eq 46 -and $totals.quick_test_count -eq 1 -and $totals.full_test_count -eq 73 -and $totals.optional_route_count -eq 8) 'TK-03 catalog distinguishes 89 owners, 46 active core checks, 73 active full checks, and 8 routes; TK-07 adds one owned R1 fixture' "catalog totals drifted: $($totals | ConvertTo-Json -Compress)"
