...
try{
    ...
    Check ($new.detected_protocol-ceq'new'-and$new.selected_protocol-ceq'v1'-and$new.reason-ceq'runtime-default-missing'-and$new.warning-match'deprecated'-and$new.side_effects.runtime_writes-eq0) 'auto without a Runtime Default keeps a new task on v1 with diagnostics and no writes' 'new auto protocol resolution is wrong'
