...
Check ($trialText-match'\$installedMode = \$BenchmarkPath -ceq ''installed-desktop-path'''-and$runnerText-match'-BenchmarkPath \$BenchmarkPath -EligibilityReportPath \$EligibilityReportPath'-and$wrapperText-match'if \(\$LoadUserConfig -and \$Isolated\) \{ throw ''-LoadUserConfig cannot be combined with -Isolated\.'' \}'-and$wrapperText-match'if \(\$LoadUserConfig\) \{ \$telemetry\[''user_config_mode''\] = ''loaded'' \}'-and@([regex]::Matches($wrapperText,'user_config_mode')).Count-eq1) 'installed path selection, mutually exclusive user-config invocation, or conditional-only telemetry contract is missing'
...
Check ($trialText-match'function Invoke-InstalledDesktopRolloutPromotion'-and$trialText-match'promote-v2-rollout-report\.ps1'-and$trialText-match'function Invoke-InstalledDesktopRouteProbe'-and$trialText-match'requested_protocol -ceq ''auto'''-and$trialText-match'selected_protocol -ceq ''v2'''-and$trialText-match'rollout_eligibility\.status -ceq ''pass'''-and$trialText.IndexOf('Invoke-InstalledDesktopRolloutPromotion -RepoRoot')-lt$trialText.IndexOf('Invoke-InstalledDesktopInstall -CodexHome')) 'canonical rollout promotion or zero-env Auto route probe contract is missing'
...
Check ($runnerText-match'if \(\$BenchmarkPath -ceq ''installed-desktop-path''\) \{'-and$runnerText-match'\$report\.schema_version = ''harness-installed-desktop-benchmark-report/v1'''-and$trialText-match"hook_trust='unknown'"-and$trialText-match"hook_callability='unknown'") 'installed report identity is not independent or hook trust/callability is overstated'
...
Check ($groupProjection-match'measurement_passed'-and$groupProjection-match'eligible'-and$groupProjection-match'\$false'-and$groupProjection-match'qualification'-and$groupProjection-match'unavailable') 'installed group can still turn passing measurements into qualification pass/eligible'
Check ($reportProjection-match'measurement_passed'-and$reportProjection-match'eligible'-and$reportProjection-match'\$false'-and$reportProjection-match'qualification'-and$reportProjection-match'unavailable'-and$runnerText-match'\$reportStatus = .*\$unavailableGroups.*''unavailable'''-and$runnerText-match'if \(\[string\]\$report\.status -ceq ''unavailable''\).*exit 2') 'installed report can still qualify or exit zero when only measurement thresholds passed'
Check ($trialText-match"host_surface=.*codex-cli-host-equivalent"-and$runnerText-match"desktop\.host_surface -cne 'codex-cli-host-equivalent'"-and$groupProjection-match"host_surface[^\r\n]*codex-cli-host-equivalent"-and$reportProjection-match"host_surface[^\r\n]*codex-cli-host-equivalent") 'installed trial/group/report does not explicitly identify the codex CLI host-equivalent surface'
...
function New-InstalledContractRecord([string]$Protocol,[int]$Trial=1,[string]$ConfigStatus='present',[object]$ConfigDigest=('sha256:'+('1'*64))){
    $desktop=[ordered]@{benchmark_path='installed-desktop-path';host_surface='codex-cli-host-equivalent';user_config_mode='loaded';profile_config=[ordered]@{status=$ConfigStatus;digest=$ConfigDigest};protocol_environment='cleared';hook_trust='unknown';hook_callability='unknown'}
    if($Protocol-ceq'bare'){$desktop+=@{install_status='not-applicable';verification_status='not-applicable';cleanup_status='not-required';hook_installed='not-applicable';auth_unchanged=$null;route_probe=$null;rollout_promotion=$null}}
    else{
        $desktop+=@{install_status='pass';verification_status='pass';cleanup_status='passed';hook_installed='verified';auth_unchanged=$true;rollout_promotion=[ordered]@{status='pass';source_revision='revision';report_digest=('sha256:'+('2'*64));file_digest=('sha256:'+('5'*64))}}
        $desktop.route_probe=if($Protocol-ceq'v2'){[ordered]@{requested_protocol='auto';selected_protocol='v2';detected_protocol='new';reason='eligible-rollout-report';rollout_status='pass';report_digest=('sha256:'+('2'*64))}}else{[ordered]@{requested_protocol='auto';selected_protocol='v1';detected_protocol='v1';reason='existing-v1-plan';rollout_status='not-required';report_digest=$null}}
    }
...
$wrongHostSurface=New-InstalledContractRecord -Protocol v2;$wrongHostSurface.installed_desktop.host_surface='desktop-hook-gui'
Check (-not(Test-HostTrialContract -Protocol v2 -Record $wrongHostSurface -ExpectedTrial 1 -SourceRevision revision -SourceCommitTree tree -BenchmarkPath installed-desktop-path)) 'runner accepts an installed trial that overstates the observed host surface'
...
$promotionSet=[ordered]@{};foreach($protocol in @('bare','v1','v2')){$promotionSet[$protocol]=[ordered]@{trials=@(1..3|ForEach-Object{New-InstalledContractRecord -Protocol $protocol -Trial $_})}}
$consistentPromotion=Test-ReleaseHostTrialSet -Records $promotionSet -RequiredTrials 3 -SourceRevision revision -SourceCommitTree tree -BenchmarkPath installed-desktop-path
$promotionSet.v1.trials[2].installed_desktop.rollout_promotion.report_digest='sha256:'+('4'*64)
Check ($consistentPromotion-and-not(Test-ReleaseHostTrialSet -Records $promotionSet -RequiredTrials 3 -SourceRevision revision -SourceCommitTree tree -BenchmarkPath installed-desktop-path)) 'runner accepts mixed rollout promotion report digests in one installed group'
$missingReportFailedClosed=$false
try { [void](Invoke-InstalledDesktopRolloutPromotion -RepoRoot $RepoRoot -Workspace $RepoRoot -EligibilityReportPath '') } catch { $missingReportFailedClosed=$_.Exception.Message -ceq 'host-benchmark-installed-rollout-report-missing' }
Check $missingReportFailedClosed 'installed benchmark accepted a missing canonical rollout report'
