...
$installedContractStart=$trialText.IndexOf('$installedContractPassed =')
$installedContractEnd=$trialText.IndexOf('if (-not $writeBoundaryPassed',$installedContractStart)
$installedContractBlock=if($installedContractStart-ge0-and$installedContractEnd-gt$installedContractStart){$trialText.Substring($installedContractStart,$installedContractEnd-$installedContractStart)}else{''}
Check ($installedContractBlock-match'\.requested_protocol -ceq \$\(if\s*\(\$Protocol\s*-ceq\s*''v2''\)\s*\{\s*''v2''\s*\}\s*else\s*\{\s*''auto''\s*\}\)'-and$installedContractBlock-match'\.selected_protocol -ceq \$Protocol') 'installed Trial completion does not accept the protocol request produced by each authoritative route probe'
...
Check ($installPreBlock-match'Get-HarnessFileSha256 -Path \$authPath'-and$installPreBlock-match'Get-InstalledDesktopUserConfigBinding'-and$installImmediateBlock-match'\$installExit\w*\s*=\s*\$LASTEXITCODE'-and$installImmediateBlock-match'Get-HarnessFileSha256 -Path \$authPath'-and$installImmediateBlock-match'Get-InstalledDesktopUserConfigBinding'-and$installImmediateBlock-match'Test-InstalledDesktopUserConfigBinding'-and$installVerifyBlock-match'Get-HarnessFileSha256 -Path \$authPath'-and$installVerifyBlock-match'Get-InstalledDesktopUserConfigBinding'-and$installVerifyBlock-match'Test-InstalledDesktopUserConfigBinding') 'installed install/verification does not compare immediate auth and config snapshots before interpreting child exit status'
