...
try {
    ...
    $expectedPaths=@('aggregator-runner-observation.json','release-isolation.json','exact-head-engineering.json','release-full-receipt.json')
    ...
    Check (($currentPaths-join'|')-ceq($expectedPaths-join'|') -and $currentUpload-match'if-no-files-found:\s*error') 'Default Promotion rollout bundle contains exactly four required files' 'Default Promotion rollout bundle scope is wrong'
