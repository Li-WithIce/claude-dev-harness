...
- Windows Fixture 覆盖 `cmd.exe /C`、PowerShell 7 和 Windows PowerShell；它只验证安装命令的 JSON 行为，不代表 Hook 已被 Host 信任/启用，也不代表企业端点策略放行
- 普通文件写入只把目标路径送入 core policy：direct `apply_patch` 严格解析全部 Add/Update/Delete/Move 目标，`Write`、`Edit`、`MultiEdit`、`NotebookEdit` 只传对应路径；配置正文不进入 `command_text`。`Bash` 仍传真实 command text，shell-form `apply_patch` / `applypatch` 继续 fail closed
