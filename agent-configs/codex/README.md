...
- Windows Hook 的确定性 Fixture 覆盖 `cmd.exe /C`、PowerShell 7 和 Windows PowerShell；它只验证安装命令的 JSON 行为，不代表 Hook 已被 Host 信任/启用，也不代表企业端点策略放行
- 普通文件写入只把目标路径送入 core policy：direct `apply_patch` 严格解析全部 Add/Update/Delete/Move 目标并按 Hook payload 的 `cwd` 做 Workspace containment，`Write`、`Edit`、`MultiEdit`、`NotebookEdit` 只传对应路径；配置正文不进入 `command_text`，未来等价工具的 adapter 也必须沿用该合同。缺少 `cwd`、无法解析目标、read-only、Workspace 外路径、受保护路径或宿主/系统拒绝仍 fail closed
- `Bash` command text 仍进入 core policy；shell-form `apply_patch` / `applypatch` 是实际 Bash 命令，当前 Hook 缺少有效 tool workdir/environment 绑定，因此继续 fail closed
- 用户明确授权后，数据库凭证和外部服务 Key 可以持久化到指定 Workspace 配置文件，包括生产配置；这不依赖精确 Host/model 版本、Release Qualification、Vault、KMS 或 Secret Provider。持久化不等于披露，回复、日志、Evidence、Review、PR 与 CI 不得回显敏感值
