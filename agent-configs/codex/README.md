...
- `Bash` command text 进入 core policy；当 Host Hook 不提供可信的实际 environment identity/cwd，或 remote primary 可能让 Hook cwd 回退到本机时，Bash shell-form 和所有 direct `apply_patch` 都 fail closed；宿主增加可信绑定前不按看似本地的 patch 路径放行
