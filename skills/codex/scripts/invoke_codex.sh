...
usage() {
  cat <<'USAGE'
Usage:
  invoke_codex.sh <task> [options]
  invoke_codex.sh -t <task> [options]
...
Examples:
  # New task (positional)
  invoke_codex.sh "Add error handling to api.ts" -f src/api.ts

  # With explicit workspace
  invoke_codex.sh "Fix the bug" -w /other/repo

  # Continue conversation
  invoke_codex.sh "Also add retry logic" --session <id>
USAGE
}
