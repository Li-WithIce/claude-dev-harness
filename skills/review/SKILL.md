...
写完最新 run 后执行 `.assistant\entry\advance-stage.ps1 -TaskId {task_id} -ExpectedStage <PLAN_REVIEW|CODE_REVIEW>`（按当前 frontmatter stage 传值，自动调用 validator）。默认走 workflow descriptor 的 `harness-default-codex` 与 `model: inherit`；切换 backend 时显式传 `-Tool`（如用 profile 同步传 `-Profile`，只有宿主需要覆盖时才传完整 `-Model`，profile 的 backend 必须等于 `-Tool`）。
